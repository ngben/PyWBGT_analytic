...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python calculate_wbgt_aus10i_hist_noleap.py <year>")
        sys.exit(1)
    ...
    BASE_DIR = f'/g/data/hq89/CCAM/output/CMIP6/DD/AUS-10i/CSIRO/{model}/historical/{experiment}/CCAM-v2203-SN/v1-r1/1hr'
    version = 'v20231206'
    ...
    dsets_shift, ocals_shift, ounits_shift = open_local_datasets(BASE_DIR, variables, version, year, True)
    if not dsets:
    ...
    for year in years:
        ...
        for month in range(1, 13):
            ...
            is_feb29 = (rsdsdir_shift['time'].dt.month == 2) & (rsdsdir_shift['time'].dt.day == 29)
            ...
            # Optional: debug print
            target_times = rsdsdir_shift['time'].where(target_mask, drop=True)
            for t in target_times.values:
                print(
                    f"Before fix ({np.datetime_as_string(t)}):",
                    float(rsdsdir_shift.sel(time=t).mean(dim=["lat", "lon"]).compute())
                )
            ...
            # Step 4: Compute difference
            rsdsdiff_shift = (rsds_shift - rsdsdir_shift).rename("rsdsdiff")
        ...
        outdir = f"/scratch/e53/bxn599/aus10i/{model}_historical"
        os.makedirs(outdir, exist_ok=True)

        tnw_outfile = f"{outdir}/tnw_AUS-10i_{model}_historical_{experiment}_CSIRO_CCAM-v2203-SN_v1-r1_1hr_{year}01010000-{year}12312300.nc"
        tg_outfile = f"{outdir}/tg_AUS-10i_{model}_historical_{experiment}_CSIRO_CCAM-v2203-SN_v1-r1_1hr_{year}01010000-{year}12312300.nc"
        wbgt_outfile = f"{outdir}/wbgt_AUS-10i_{model}_historical_{experiment}_CSIRO_CCAM-v2203-SN_v1-r1_1hr_{year}01010000-{year}12312300.nc"
